...
async function runTool(label, action) {
    try {
        return action();
    }
    catch (error) {
        return asErrorContent(`Lỗi khi ${label}: ${getErrorMessage(error)}`);
    }
}
...
server.registerTool("list_equipment", {
...
}, async ({ limit, type_id }) => runTool("lấy danh sách thiết bị", async () => {
    ...
    const [rows] = await withDb((db) => db.query(query, params));
...
server.registerTool("search_equipment", {
...
}, async ({ query }) => runTool("tìm kiếm thiết bị", async () => {
    ...
    const [rows] = await withDb((db) => db.query(sql, [searchTerm, searchTerm]));
    return asTextContent(rows);
}));
...
}, async ({ barcode }) => runTool("kiểm tra trạng thái thiết bị", async () => {
    const query = `
...
}, async ({ limit }) => runTool("lấy lịch sử mượn trả", async () => {
    const sql = `
      SELECT 
        rt.rented_date, 
        rt.ticket_no, 
      ...
      LIMIT ?
    `;
    const [rows] = await withDb((db) => db.query(sql, [safeNumberLimit(limit)]));
    return asTextContent(rows);
}));
async function main() {
    ...
    if (dbHealth.connected) {
        ...
        console.error(`CECICS MCP Server running on stdio (degraded mode): ${dbHealth.message}`);
    }
}
main().catch((error) => {
