...
async function verifyDbConnection() {
    try {
        ...
        return { connected: true as const, message: "MySQL connected" };
    } catch (error) {
        return {
...
server.registerTool(
    ...
    async ({ limit, type_id }) =>
        runTool("lấy danh sách thiết bị", async () => {
...
server.registerTool(
    ...
    async ({ query }) =>
        runTool("tìm kiếm thiết bị", async () => {
...
server.registerTool(
    ...
    async ({ barcode }) =>
        runTool("kiểm tra trạng thái thiết bị", async () => {
            const query = `
...
server.registerTool(
    ...
    async ({ limit }) =>
        runTool("lấy lịch sử mượn trả", async () => {
            const sql = `
      SELECT 
        rt.rented_date, 
        rt.ticket_no, 
